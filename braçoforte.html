...
<body>
  ...
  <script>
    ...
    // Contador sequencial para o número do pedido
    ...
    function gerarPDF() {
      ...
      // Número do pedido
      doc.setFontSize(12);
      doc.text(`Número do Pedido: ${numeroPedido}`, 150, 20);
      numeroPedido++; // Incrementa o número do pedido

      // Título
      doc.setFontSize(18);
      doc.text("Orçamento de Coberturas", 70, 30);

      // Informações da Empresa
      doc.setFontSize(12);
      doc.text("EMPRESA DE COBERTURAS DE AÇOS DO KELVYN", 10, 50);
      doc.text("Rua Doutor Lucio Machado 15, São João de Meriti", 10, 60);
      doc.text("Orçamentos: (21) 98130-4519 grátis!", 10, 70);
      doc.text("Instagram: @bracofortecoberturas", 10, 80);

      // Tabela de detalhes do orçamento
      doc.setFontSize(14);
      doc.text("Detalhes do Orçamento:", 10, 100);
      doc.autoTable({
        startY: 110,
        head: [['Descrição', 'Valor']],
        body: [
          ['Área do telhado', `${document.getElementById("areaTotal").textContent} m²`],
          ['Quantidade de telhas', document.getElementById("quantidadeTelhas").textContent],
      ...
      doc.setFontSize(10);
      doc.text("O desenvolvimento de quantidades deve ser considerado meramente indicativo. Braço Forte Estruturas Metálicas não é responsável por quaisquer erros. O cliente ainda deve verificar, com base nessa cotação, a correspondência com suas necessidades.", 10, doc.autoTable.previous.finalY + 20, { maxWidth: 180 });

      // Logotipo e informações no final
      doc.addImage(logoUrl, "JPEG", 10, doc.autoTable.previous.finalY + 40, logoWidth, logoHeight);
      doc.setFontSize(10);
      doc.text("Braço Forte Estruturas Metálicas", 70, doc.autoTable.previous.finalY + 50);
      doc.text("Rua Doutor Lucio Machado 15, São João de Meriti - Rio de Janeiro", 70, doc.autoTable.previous.finalY + 60);
      doc.text("Tel: +55 21 98130-4519", 70, doc.autoTable.previous.finalY + 70);
      doc.text("https://www.instagram.com/bracofortecoberturas/", 70, doc.autoTable.previous.finalY + 80);
